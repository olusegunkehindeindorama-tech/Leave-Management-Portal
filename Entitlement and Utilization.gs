 ...
 *     Lookup is by SHEET NAME only (never by sheet/gid id).
 ...
 *     2025 Balance is STATIC (manually seeded) and is never overwritten.
 * ============================================================
 */
...
function loadEntitlementMasterData_() {
  ...
  // Multiplier_Policy by NAME only — structure: Shift | Yes | No
  ...
  if (sh) {
    ...
    if (mData.length > 1) {
      ...
      for (var mr = 1; mr < mData.length; mr++) {
        var code = String(mData[mr][codeIdx] || '').trim().toUpperCase();
        if (!code) continue;
        if (yesIdx >= 0) out.Yes[code] = Number(mData[mr][yesIdx]);
        if (noIdx >= 0) out.No[code] = Number(mData[mr][noIdx]);
      }
    }
  }
  if (typeof cachePut_ === 'function') cachePut_('mult_pol', out);
  return out;
}
...
function splitLeaveByCarryForward_(startDate, endDate, leaveType, pol) {
  ...
  segments.push({ start: s, end: preEnd, entitlementYear: s.getFullYear() - 1 });
...
/** Initial year seeded manually — never regenerate or overwrite this column. */
var STATIC_STARTING_BAL_YEAR = 2025;

function generateYearEndStartingBalances(optYear) {
  var today = new Date();
  var yearEnded = optYear != null ? Number(optYear) : (today.getFullYear() - 1);

  // 2025 Balance was manually imported (no 2025 leaves in tblLeave). Keep it static.
  if (yearEnded === STATIC_STARTING_BAL_YEAR) {
    return {
      success: false,
      message: 'Refused: ' + STATIC_STARTING_BAL_YEAR +
        ' Balance is the initial manually-seeded column and must not be regenerated or overwritten. ' +
        'Pass a later year (e.g. 2026) or wait for the Jan 1 trigger after that year ends.',
      column: STATIC_STARTING_BAL_YEAR + ' Balance',
      written: 0
    };
  }
  if (yearEnded < STATIC_STARTING_BAL_YEAR) {
    return {
      success: false,
      message: 'Refused: year ' + yearEnded + ' is before the static baseline year ' + STATIC_STARTING_BAL_YEAR + '.',
      written: 0
    };
  }
  ...
  Object.keys(master.employees).forEach(function (id) {
    ...
    // Update ONLY the target year column — leave 2025 Balance and other years untouched
    if (existing[id]) {
      ...
      var staticIdx = headers.indexOf(STATIC_STARTING_BAL_YEAR + ' Balance');
      if (staticIdx >= 0 && master.startingBal[id]) {
        var staticVal = master.startingBal[id][STATIC_STARTING_BAL_YEAR + ' Balance'];
        if (staticVal !== '' && staticVal !== undefined && staticVal !== null) {
          newRow[staticIdx] = staticVal;
        }
      }
  ...
  return {
    success: true,
    message: 'StartingBal column "' + colName + '" updated for ' + written +
      ' employee(s). Column "' + STATIC_STARTING_BAL_YEAR + ' Balance" was left unchanged.',
    column: colName,
    written: written
  };
}
...
/** @deprecated — multipliers come from Multiplier_Policy sheet */
