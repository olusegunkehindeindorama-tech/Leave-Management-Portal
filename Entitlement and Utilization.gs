...
/**
 * Lightweight date-range utilization helper used by the UI submission/edit flow.
 * It counts rostered working days when tblShift is available and otherwise falls
 * back to inclusive calendar days. Off, rest, and leave roster codes are skipped.
 */
function calculateLeaveUtilize(empId, startDate, endDate) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var shiftSheet = ss.getSheetByName('tblShift');
  ...
  var inclusiveDays = Math.round((eDate.getTime() - sDate.getTime()) / (1000 * 60 * 60 * 24)) + 1;
  if (!shiftSheet) return inclusiveDays;

  var shiftData = shiftSheet.getDataRange().getValues();
  if (shiftData.length < 2) return inclusiveDays;

  var shiftHeaders = shiftData[0].map(function(h) { return String(h).trim(); });
  var empIdx = getHeaderIndex_(shiftHeaders, ["Emp ID", "Employee ID", "Emp No"]);
  var dateIdx = getHeaderIndex_(shiftHeaders, ["Date", "Shift Date"]);
  var codeIdx = getHeaderIndex_(shiftHeaders, ["Shift", "Shift Code", "Code"]);
  ...
  var offCodes = { "OFF": true, "REST": true, "R": true, "WO": true, "W/O": true, "LEAVE": true, "L": true };
  var rosteredDates = {};

  for (var i = 1; i < shiftData.length; i++) {
    if (String(shiftData[i][empIdx]).trim().toUpperCase() !== target) continue;
    var dateObj = new Date(shiftData[i][dateIdx]);
    if (isNaN(dateObj.getTime()) || dateObj < sDate || dateObj > eDate) continue;
    var code = String(shiftData[i][codeIdx]).trim().toUpperCase();
    if (!offCodes[code]) rosteredDates[formatDateKey(dateObj)] = true;
  }

  var counted = Object.keys(rosteredDates).length;
  return counted > 0 ? counted : inclusiveDays;
}
